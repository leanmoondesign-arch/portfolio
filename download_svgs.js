...
https.get('https://svgl.app/api/svgs', (res) => {
  ...
  res.on('end', () => {
    ...
    tools.forEach(toolName => {
      const tool = svgs.find(s => s.title.toLowerCase() === toolName.toLowerCase());
      if (tool) {
        // Download logotype (usually wordmark)
        if (tool.wordmark) {
          download(tool.wordmark, path.join(logotypesDir, `${tool.title.toLowerCase().replace(/ /g, '_')}.svg`));
        } else {
          // Fallback to route if no wordmark
          download(typeof tool.route === 'object' ? tool.route.light : tool.route, path.join(logotypesDir, `${tool.title.toLowerCase().replace(/ /g, '_')}.svg`));
        }
        
        // Download isotype
        const isotypeUrl = typeof tool.route === 'object' ? tool.route.light : tool.route;
        download(isotypeUrl, path.join(isotypesDir, `${tool.title.toLowerCase().replace(/ /g, '_')}.svg`));
      } else {
